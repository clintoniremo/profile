...
Over the past five years, I have gained extensive experience in accounting, auditing, and financial leadership across various sectors including parastatals, real estate, non-governmental organizations (NGOs), healthcare facilities, and technology firms. My experience includes spearheading external financial reconstructions, implementing robust internal controls, managing payroll allocations for 150+ staff, and securing four consecutive clean audit reports.
...
${PROFILE.phone} | ${PROFILE.email}`;
  ...
  try {
    // Build attachments array, include any extraAttachments if provided
    ...
    // Merge any additional attachments (e.g., poster image) passed via paths.extraAttachments
    const allAttachments = paths.extraAttachments
      ? baseAttachments.concat(paths.extraAttachments)
      : baseAttachments;
...
// Entry point removed; server uses Express API

// ---------- EXPORTS & UTILITIES ----------
// Export functions for the Express server integration
module.exports = {
  fetchOnlineJobs,
...
/**
 * mergeDocuments - Combine CV PDF (or HTML) with a cover letter text into a single PDF.
 * For simplicity, if a compiled PDF CV already exists, we just return its path.
 * In a full implementation, you could use pdf-lib to embed the cover letter as the first page.
 */
async function mergeDocuments(cvPath, coverPath) {
  // cvPath may be a PDF (generated by compilePdfFromHtml) or an HTML file.
  // coverPath is a text file. We'll simply return the existing PDF if present.
  if (!cvPath) {
    throw new Error('cvPath is required for merging documents');
  }
  // If the CV is already a PDF, just use it.
  ...
  // If PDF conversion fails, fallback to original CV path.
  return cvPath;
}
  ...
  if (hasDashboard) {
  console.log('Dashboard server is managed by Express (server.js).');
