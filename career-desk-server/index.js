...
function createStore(initial,{directory=process.env.CAREER_DESK_DATA_DIR||path.join(__dirname,'../.career-data'),request=global.fetch}={}){
  ...
  function postgres(){
    ...
    if(!databaseReady)databaseReady=(async()=>{await sql.query('CREATE TABLE IF NOT EXISTS zuriel_career_desk_state (id TEXT PRIMARY KEY, state JSONB NOT NULL)');await sql.query('INSERT INTO zuriel_career_desk_state (id,state) VALUES ($1,$2::jsonb) ON CONFLICT (id) DO NOTHING',[key,JSON.stringify(initial)]);})();
    return {sql,ready:databaseReady};
  }
  ...
  function filename(){if(process.env.VERCEL)throw Error('Configure the connected database for durable hosted storage.');fs.mkdirSync(directory,{recursive:true});return path.join(directory,'state.json');}
  return {
    async read(){const db=postgres();if(db){await db.ready;const rows=await db.sql.query('SELECT state FROM zuriel_career_desk_state WHERE id=$1',[key]);return rows[0].state;}if(url&&token){await redis(['SET',key,JSON.stringify(initial),'NX']);return JSON.parse(await redis(['GET',key]));}const file=filename();try{return JSON.parse(fs.readFileSync(file,'utf8'));}catch(e){if(e.code!=='ENOENT')throw e;fs.writeFileSync(file,JSON.stringify(initial),{flag:'wx',mode:0o600});return structuredClone(initial);}},
    async write(state){const next={data:state.data,revision:state.revision+1};const db=postgres();if(db){await db.ready;const rows=await db.sql.query("UPDATE zuriel_career_desk_state SET state=$1::jsonb WHERE id=$2 AND (state->>'revision')::int=$3 RETURNING id",[JSON.stringify(next),key,state.revision]);return rows.length===1;}if(url&&token){const script="local s=redis.call('GET',KEYS[1]); if not s or cjson.decode(s).revision~=tonumber(ARGV[1]) then return 0 end; redis.call('SET',KEYS[1],ARGV[2]); return 1";return await redis(['EVAL',script,1,key,state.revision,JSON.stringify(next)])===1;}
